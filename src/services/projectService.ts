...
`;
	...
	private defaultMakefile(project: Mspm0ProjectFile, device: DeviceInfo): string {
		...
		// Windows mingw32-make may use cmd/CreateProcess (not sh). Force cmd on Windows_NT
		// and keep POSIX helpers for non-Windows. Quote tool/SDK paths for spaces.
		const flashCmd =
			probe === 'jlink'
